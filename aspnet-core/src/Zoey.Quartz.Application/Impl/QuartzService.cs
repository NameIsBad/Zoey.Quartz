...
namespace Zoey.Quartz.Application
{
    public class QuartzService : IQuartzService
    {
        ...
        /// <returns></returns>
        public async Task<PagingQueryResult<TaskOptions>> GetJobs(TaskOptionsQueryFilter queryFilter)
        {
            List<TaskOptions> list = new List<TaskOptions>();
            var tasks = await _task.GetPage(queryFilter);
            return tasks;

            //TODO:CheckExists??
            var _scheduler = await _schedulerFactory.GetScheduler();
            var allTask = (await _task.GetPage(queryFilter)).DataList;
