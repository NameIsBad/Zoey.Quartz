...
namespace Zoey.Quartz.Web.Controllers
{
    public class TaskBackGroundController : Controller
    {
        ...
        {
            return Json(await _quartzService.GetJobs());
        }
        /// <summary>
        /// 获取作业运行日志
        /// </summary>
        ...
        /// <returns></returns>
        public async Task<IActionResult> GetRunLog(int taskId, int page = 1)
        {
            System.Collections.Generic.IEnumerable<Domain.Models.TaskLog> result = await _quartzLog.GetJobRunLog(taskId, page);
            return Json(result);
        }
        /// <summary>
        /// 添加任务
